...
class FindApp extends EPlugin {
    name = 'FindApp';
    quick = '*';

    async query(query: string): Promise<EPluginResult[]> {
        if (!initialized) return this.initAndGetData(query);
        return this.getData(query)
    }

    getData(query: string): Promise<EPluginResult[]> {
    ...
    async initAndGetData(query: string): Promise<EPluginResult[]> {
        initialized = true;
        ...
        if (cacheFileExist) {
            ...
            CommonUtils.createFolder(cachePath)
        }
        return this.getData(query)
    }
}
